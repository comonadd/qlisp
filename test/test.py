...
if os.name == "nt":
    INTERP_PATH = os.path.join(BDIR, "qlisp.exe")
elif os.name == "posix":
    INTERP_PATH = os.path.join(BDIR, "qlisp")
else:
...
def main():
    ...
    print(
        "{}{} {} succeeded{}".format(
            COL_OKGREEN, succeeded, fmt_num_word("test", succeeded), COL_ENDC
        )
    )
    if failed != 0:
        print(
            "{}{} {} failed{}".format(
                COL_FAIL, failed, fmt_num_word("test", failed), COL_ENDC
            )
        )
